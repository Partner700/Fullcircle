...
function PlayerAvatar({ player, size = 'sm' }: { player: RoadHomePlayer; size?: 'sm' | 'lg' }) {
  ...
  return <span className={cn('relative flex flex-shrink-0 items-center justify-center font-bold', dimensions)}><UserAvatar userId={player.isBot ? `bot-${player.id}` : player.id} name={player.name} avatarUrl={player.avatarUrl} className={cn('h-full w-full border-2 shadow-sm', COLOUR_CLASS[player.colour])} /><VallumAvatarBadge userId={player.isBot ? null : player.id} size={size === 'lg' ? 'md' : 'sm'} /></span>;
}
...
function PlayerStrip({ state, userId }: { state: RoadHomeState; userId: string }) {
  return <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{state.players.map((player, index) => {
    ...
    return <div key={player.id} className={cn('min-w-0 rounded-lg border bg-surface/90 p-2.5 transition-all', active ? 'border-gold shadow-md' : 'border-border')}><div className="flex items-center gap-2"><PlayerAvatar player={player} /><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-ink">{player.name}{player.id === userId ? ' · You' : ''}</p><div className="mt-0.5 flex gap-2 text-[10px] text-stone"><span title="Temporary Road Home currency">{player.matchDenarii ?? player.denarii ?? 0} MD</span><span>{home}/4 home</span></div></div>{active && <span className="rounded-full bg-gold-soft px-1.5 py-0.5 text-[8px] font-bold uppercase text-gold">Turn</span>}</div></div>;
  })}</div>;
}
