  ...
  return (
    <div className="rounded-xl border border-border bg-surface-2 p-3">
      ...
      {(() => {
        ...
        return (
          <div className="mt-2 flex items-center -space-x-1.5" aria-label={`${actors.length} camp members reacted`}>
            {actors.map((actor) => (
              <span key={actor.user_id} title={actor.display_name} className="inline-flex h-5 w-5 overflow-hidden rounded-full border border-surface-2 bg-peri-soft text-center text-[8px] font-bold leading-5 text-peri shadow-sm">
                {actor.avatar_url
            ...
            return (
              <div key={comment.id} className={cn('rounded-lg bg-surface/75 p-2', comment.parent_comment_id && 'ml-3')}>
                <div className="flex items-start gap-2">
                  <span className="inline-flex h-6 w-6 shrink-0 overflow-hidden rounded-full border border-border bg-peri-soft text-center text-[9px] font-bold leading-6 text-peri">
