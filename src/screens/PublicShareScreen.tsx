  ...
  return (
    <div className="rounded-xl border border-border bg-surface-2 p-3">
      ...
      {(() => {
        ...
        return (
          <div className="mt-2 flex items-center -space-x-1" aria-label={`${actors.length} camp members reacted`}>
            {actors.map((actor) => (
              <span key={actor.user_id} title={actor.display_name} className="inline-flex h-4 w-4 overflow-hidden rounded-full border border-surface-2 bg-peri-soft text-center text-[6px] font-bold leading-4 text-peri shadow-sm">
                {actor.avatar_url
