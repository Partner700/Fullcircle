    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[190] flex items-center justify-center bg-black/55 p-3 sm:p-5" role="dialog" aria-modal="true" aria-label={`${quote.display_name}'s meditation`} onMouseDown={() => setOpen(false)}>
          <article className="card relative isolate max-h-[86vh] w-full max-w-xl overflow-hidden border-border shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="relative z-10 max-h-[calc(86vh-5rem)] overflow-y-auto overscroll-contain px-5 py-5">
