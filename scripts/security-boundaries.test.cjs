...
const nonBlockingErrorBoundary = read('src/components/NonBlockingErrorBoundary.tsx');
const clientErrorReporting = read('src/lib/clientErrorReporting.ts');
const clientErrorReportsMigration = read('supabase/migrations/20260925100000_authenticated_client_error_reports.sql');
const safeStorage = read('src/lib/safeStorage.ts');
const soundscape = read('src/lib/soundscape.ts');
...
const tenCadetsPerTentAndShanaAssignment = read('supabase/migrations/20260924130000_ten_cadets_per_tent_and_shana_assignment.sql');
...
assert.match(quizResponders, /visibleBoard\.map\(\(member\)/);
...
assert.match(fcxExperience, /min-w-0 whitespace-nowrap text-center text-sm font-black tabular-nums/);
...
assert.match(profileCvModal, /bg-bg\/\[0\.45\]/);
...
assert.match(indexCss, /\.message-space-veil,[\s\S]*\.profile-cv-veil \{[\s\S]*rgba\(7, 20, 43, 0\.34\)/);
...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v151'/);
assert.match(serviceWorker, /CACHE_STORAGE_VERSION = 'full-circle-v147-v151'/);
assert.match(serviceWorker, /ROLLBACK_CACHE_PREFIXES = \['full-circle-v147-v150', 'full-circle-v147-v149', 'full-circle-v148'\]/);
assert.match(serviceWorker, /RECOVERY_MARKER = '142'/);
...
assert.match(serviceWorker, /async function fetchReleaseWithFallback/);
...
assert.match(serviceWorker, /function criticalReleaseFiles/);
assert.match(serviceWorker, /entry && entry\.dynamicImports/);
assert.match(serviceWorker, /const cached = await caches\.match\(request, \{ ignoreSearch: true/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '125'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=128`/);
...
assert.match(releaseCache, /2026-09-25-v151/);
assert.match(releaseCache, /'full-circle-v147-v151', 'full-circle-v147-v150', 'full-circle-v147-v149', 'full-circle-v148'/);
...
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=128'/);
...
assert.match(read('public/manifest.webmanifest'), /"start_url": "\.\/\?fc-launch=124"/);
...
assert.match(authContext, /IDENTITY_CACHE_PREFIX = 'full-circle-identity:'/);
assert.match(authContext, /readCachedIdentity\(data\.session\.user\.id\)/);
assert.match(authContext, /waitForAbortable/);
assert.match(supabaseClient, /async function resilientFetch/);
assert.match(supabaseClient, /const attempts = canRetry \? 2 : 1/);
...
assert.match(orderedAnswersAndMarketContinuity, /CREATE OR REPLACE FUNCTION public\.quiz_answer_is_correct/);
assert.equal((orderedAnswersAndMarketContinuity.match(/normalize_order_sequence_answer\(/g) || []).length >= 7, true);
assert.match(orderedAnswersAndMarketContinuity, /room\.status = 'waiting'/);
assert.match(orderedAnswersAndMarketContinuity, /DELETE FROM public\.arena_question_decks/);
assert.match(reliableTentDeletion, /CREATE OR REPLACE FUNCTION public\.delete_tent/);
assert.match(reliableTentDeletion, /UPDATE public\.streakboard_snapshots[\s\S]*SET tent_id = NULL/);
assert.match(reliableTentDeletion, /UPDATE public\.leaderboard_weekly_snapshots[\s\S]*SET tent_id = NULL/);
assert.match(reliableTentDeletion, /assignment\.role = 'instructor'/);
assert.match(reliableTentDeletion, /GRANT EXECUTE ON FUNCTION public\.delete_tent\(uuid\) TO authenticated, service_role/);
assert.match(instructorApp, /That tent no longer exists/);
assert.match(instructorApp, /was deleted\. Its members are now available for reassignment/);
...
assert.match(arenaGenerator, /type === "standard_text" && !isSingleWordAnswer\(correctAnswer\)/);
assert.doesNotMatch(arenaGenerator, /OPENAI_API_KEY|chat\/completions|FALLBACK_FACTS|ARENA_BANKS/);
assert.match(roadHomeEngine, /type === 'standard_text' && correctAnswer\.split\(\/\\s\+\//);
...
assert.match(quoteReactions, /comments\.length > 0 && 'quote-comments-boundary-fade max-h-72 pb-24'/);
assert.match(cadetDashboard, /min\(82svh, calc\(100dvh - 7rem\)\)/);
assert.match(cadetDashboard, /height: conversationOpen \? panelMaxHeight : undefined/);
assert.match(confirmedPresenceStreak, /coalesce\(record\.attendance_status, 'unmarked'\) = 'present'\s+OR EXISTS/);
assert.match(confirmedPresenceStreak, /v_affected_date date := date '2026-09-08'/);
...
assert.match(cadetStore, /refreshMutableStoreState/);
assert.match(cadetStore, /Purchase complete\. Your relic has been added to your inventory\./);
assert.doesNotMatch(cadetStore, /Payment was not confirmed within 35 seconds\. No relic was added/);
assert.match(rootApp, /const AuthenticatedOverlays = lazy/);
assert.match(rootApp, /NonBlockingErrorBoundary name="authenticated overlays"/);
...
assert.match(clientErrorReporting, /CLIENT_RELEASE = '2026-09-25-v151'/);
...
assert.match(clientErrorReportsMigration, /CREATE OR REPLACE FUNCTION public\.report_client_error/);
assert.match(clientErrorReportsMigration, /v_user_id uuid := auth\.uid\(\)/);
assert.match(clientErrorReportsMigration, /GRANT EXECUTE ON FUNCTION public\.report_client_error/);
assert.match(safeStorage, /function browserStorage/);
assert.match(safeStorage, /safeJsonStorageGet/);
assert.match(appShell, /safeStorageGet\('local', 'fc-theme'\)/);
assert.match(soundscape, /safeStorageGet\('local', SOUND_ENABLED_KEY\)/);
assert.doesNotMatch(soundscape, /localStorage\.(?:getItem|setItem)/);
...
for (const required of [
  'UPDATE public.tents',
  'SET max_cadets = 10',
  'CREATE OR REPLACE FUNCTION public.tent_cadet_limit',
  'CREATE OR REPLACE FUNCTION public.tent_cadet_count',
  "coalesce(member.role, 'cadet') = 'cadet'",
  'BEFORE INSERT OR UPDATE OF tent_id, user_id, role ON public.tent_members',
  'maximum % cadets plus its sentry',
  'CREATE OR REPLACE FUNCTION public.assign_cadet_to_tent',
  'CREATE OR REPLACE FUNCTION public.sentry_assign_cadet_to_tent',
  'CREATE OR REPLACE FUNCTION public.get_sentry_addable_cadets',
  'CREATE OR REPLACE FUNCTION public.request_to_join_tent',
  'CREATE OR REPLACE FUNCTION public.review_tent_join_request',
  "LIKE 'shana%'",
  "IN ('squarea0', 'squaresa0', 'a0')",
]) {
  assert.ok(tenCadetsPerTentAndShanaAssignment.includes(required), `Missing ten-cadet or Shana assignment safeguard: ${required}`);
}
assert.doesNotMatch(tenCadetsPerTentAndShanaAssignment, /maximum 10 people/);
assert.match(instructorApp, /const TENT_CADET_CAPACITY = 10/);
assert.match(instructorApp, /member\.role === 'cadet'/);
assert.match(instructorApp, /sentry assigned separately/);
assert.doesNotMatch(instructorApp, /const TENT_PERSON_CAPACITY/);
...
assert.match(fcxExperience, /aria-label="Pay for the Full Circle Experience"/);
assert.match(fcxExperience, /CircleDollarSign size=\{16\}/);
assert.match(fcxExperience, />Pay<\/span>/);
assert.match(fcxExperience, /aria-label="Ask about the Full Circle Experience"/);
assert.match(fcxExperience, /MessageCircle size=\{16\}/);
assert.match(fcxExperience, />Ask<\/span>/);
assert.match(fcxExperience, /bg-\[#ffd84d\] text-\[#07182b\]/);
assert.match(fcxExperience, /bg-white text-\[#07182b\]/);
assert.match(fcxExperience, /flex h-12 max-w-full items-stretch/);
assert.match(fcxExperience, /overflow-visible[\s\S]*awardBadgeSize="xs"/);
assert.match(cadetDashboard, /variant="podium"[\s\S]*competitorRole=\{slide\.division === 'Cadets' \? 'cadet' : 'sentry'\}/);
assert.match(quizResponders, /variant\?: 'card' \| 'slide' \| 'podium'/);
assert.match(quizResponders, /opacity-60 grayscale/);
assert.match(quizResponders, /responders\.map\(\(member\) => participantAvatar\(member, true\)\)/);
assert.match(quizResponders, /waitingMembers\.map\(\(member\) => participantAvatar\(member, false\)\)/);
assert.match(quizResponders, /h-\[22px\] w-\[22px\]/);
