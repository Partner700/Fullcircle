...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v143'/);
assert.match(serviceWorker, /RECOVERY_MARKER = '136'/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '122'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=122`/);
assert.match(serviceWorkerRegistration, /postMessage\(\{ type: 'WARM_APP_SHELL' \}\)/);
assert.match(staleBundleRecovery, /set\('fc-release', '122'\)/);
assert.doesNotMatch(staleBundleRecovery, /window\.caches\.delete/);
assert.match(staleBundleRecovery, /lastRecoveryInMemory/);
assert.match(releaseCache, /2026-09-20-v143/);
assert.match(releaseCache, /mobile privacy mode blocks storage/);
assert.match(appIndex, /%BASE_URL%manifest\.webmanifest\?v=119/);
assert.match(appIndex, /var release = '120'/);
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=122'/);
...
assert.match(read('public/manifest.webmanifest'), /"start_url": "\.\/\?fc-launch=119"/);
...
assert.match(quoteQueries, /getDayType\(getTodayISODate\(\)\) === 'weekday' \? quotes : quotes\.slice\(0, 1\)/);
...
assert.match(cadetDashboard, /\{ id: 'welcome', kind: 'welcome' \},[\s\S]{0,80}\.\.\.weekendQuoteSlides/);
...
assert.match(sentryApp, /veilClassName: 'welcome-first-slide-veil',[\s\S]{0,80}\.\.\.weekendQuoteSlides/);
