...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v139'/);
assert.match(serviceWorker, /RECOVERY_MARKER = '132'/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '118'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=118`/);
assert.match(serviceWorkerRegistration, /postMessage\(\{ type: 'WARM_APP_SHELL' \}\)/);
assert.match(staleBundleRecovery, /set\('fc-release', '118'\)/);
assert.doesNotMatch(staleBundleRecovery, /window\.caches\.delete/);
assert.match(staleBundleRecovery, /lastRecoveryInMemory/);
assert.match(releaseCache, /2026-09-20-v139/);
assert.match(releaseCache, /mobile privacy mode blocks storage/);
assert.match(appIndex, /%BASE_URL%manifest\.webmanifest\?v=115/);
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=118'/);
...
assert.match(read('public/manifest.webmanifest'), /"start_url": "\.\/\?fc-launch=115"/);
...
assert.match(quizResponders, /opacity-40 grayscale/);
