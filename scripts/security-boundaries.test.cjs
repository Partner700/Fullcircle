...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v131'/);
assert.match(serviceWorker, /RECOVERY_MARKER = '124'/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '110'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=110`/);
assert.match(staleBundleRecovery, /set\('fc-release', '110'\)/);
assert.match(staleBundleRecovery, /lastRecoveryInMemory/);
assert.match(releaseCache, /2026-09-16-v131/);
assert.match(releaseCache, /mobile privacy mode blocks storage/);
assert.match(appIndex, /%BASE_URL%manifest\.webmanifest\?v=109/);
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=110'/);
