...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v137'/);
assert.match(serviceWorker, /RECOVERY_MARKER = '130'/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '116'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=116`/);
assert.match(serviceWorkerRegistration, /postMessage\(\{ type: 'WARM_APP_SHELL' \}\)/);
assert.match(staleBundleRecovery, /set\('fc-release', '116'\)/);
assert.doesNotMatch(staleBundleRecovery, /window\.caches\.delete/);
assert.match(staleBundleRecovery, /lastRecoveryInMemory/);
assert.match(releaseCache, /2026-09-19-v137/);
assert.match(releaseCache, /mobile privacy mode blocks storage/);
assert.match(appIndex, /%BASE_URL%manifest\.webmanifest\?v=113/);
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=116'/);
...
assert.match(read('public/manifest.webmanifest'), /"start_url": "\.\/\?fc-launch=113"/);
