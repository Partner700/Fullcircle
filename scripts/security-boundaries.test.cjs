...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v93'/);
assert.match(serviceWorker, /RECOVERY_MARKER = '93'/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '93'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=93`/);
assert.match(staleBundleRecovery, /set\('fc-release', '93'\)/);
assert.match(releaseCache, /2026-08-25-v93/);
assert.match(appIndex, /%BASE_URL%manifest\.webmanifest\?v=93/);
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=93'/);
assert.match(read('public/manifest.webmanifest'), /"start_url": "\.\/\?fc-launch=93"/);
