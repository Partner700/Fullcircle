...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v132'/);
assert.match(serviceWorker, /RECOVERY_MARKER = '125'/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '111'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=111`/);
assert.match(staleBundleRecovery, /set\('fc-release', '111'\)/);
assert.match(staleBundleRecovery, /lastRecoveryInMemory/);
assert.match(releaseCache, /2026-09-17-v132/);
assert.match(releaseCache, /mobile privacy mode blocks storage/);
assert.match(appIndex, /%BASE_URL%manifest\.webmanifest\?v=110/);
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=111'/);
