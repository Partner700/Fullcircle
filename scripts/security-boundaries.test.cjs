...
assert.match(serviceWorker, /CACHE_VERSION = 'full-circle-v90'/);
assert.match(serviceWorker, /RECOVERY_MARKER = '90'/);
...
assert.match(offlinePage, /RECOVERY_VERSION = '90'/);
...
assert.match(serviceWorkerRegistration, /register\(`\$\{import\.meta\.env\.BASE_URL\}sw\.js\?v=90`/);
assert.match(staleBundleRecovery, /set\('fc-release', '90'\)/);
assert.match(releaseCache, /2026-08-25-v90/);
assert.match(appIndex, /%BASE_URL%manifest\.webmanifest\?v=90/);
assert.match(appIndex, /register\('%BASE_URL%sw\.js\?v=90'/);
assert.match(read('public/manifest.webmanifest'), /"start_url": "\.\/\?fc-launch=90"/);
