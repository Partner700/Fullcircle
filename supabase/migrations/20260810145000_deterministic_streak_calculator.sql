/*
  Define streak behavior directly instead of rewriting older function text.

  Rules:
  - Monday-Friday: attendance plus meditation, both within their deadlines.
  - Sentries satisfy attendance by marking at least one assigned cadet before noon.
  - Saturday: a submitted/timed-out Saturday quiz when one was released.
  - Sunday: opening Today's Reading before 9 PM, from 2 August 2026 onward.
  - Only protection rows already assigned to a date can repair that date.
  - Simon's Purse cannot protect Saturday or Sunday.
  - Thief's Request may repair a broken chain; Redemption Coin may rebuild it
    from the first eligible day even when the current chain is zero.
*/
...
BEGIN
  ...
  END IF;
  IF p_record_date IS DISTINCT FROM v_local_now::date
    OR extract(dow FROM p_record_date) <> 0 THEN
    RETURN false;
  END IF;
  ...
  WHILE v_check <= p_cutoff_date LOOP
    IF extract(dow FROM v_check) = 0 THEN
      IF v_check < date '2026-08-02' THEN
        ...
        CONTINUE;
      END IF;
    ELSIF extract(dow FROM v_check) = 6 AND NOT EXISTS (
...
BEGIN
  ...
  WHILE v_check <= v_today LOOP
    ...
    IF extract(dow FROM v_check) = 0 THEN
      -- Sundays before the reading-open rule existed remain neutral.
      IF v_check < date '2026-08-02' THEN
        v_check := v_check + 1;
        CONTINUE;
      END IF;
    ...
    ELSIF extract(dow FROM v_check) = 6 THEN
      ...
      -- A Saturday without a released quiz cannot count against a user.
...
/* Re-audit historical Thief's Request repairs. An older repair migration used
 * the latest missed date rather than the actual chain-breaking date, so rows
 * marked "restored" must be verified too. Every repair remains bounded by the
 * original use date and can never repair a later loss. */
DO $$
DECLARE
  v_repair record;
  v_cutoff date;
...
BEGIN
  FOR v_repair IN
    SELECT
      repair.usage_ledger_id,
      repair.user_id,
      repair.applied_to_date,
      repair.outcome,
      repair.created_at AS repair_created_at,
      ledger.created_at
    FROM public.relic_recovery_repairs repair
    JOIN public.denarii_ledger_entries ledger ON ledger.id = repair.usage_ledger_id
    WHERE repair.outcome IN ('no_recoverable_gap', 'restored')
  LOOP
    v_cutoff := (v_repair.created_at AT TIME ZONE 'Africa/Douala')::date
      - CASE
          WHEN (v_repair.created_at AT TIME ZONE 'Africa/Douala')::time >= time '21:00' THEN 0
          ELSE 1
        END;
    v_restore_date := public.find_latest_recoverable_streak_gap(v_repair.user_id, v_cutoff);
    IF v_restore_date IS NULL THEN CONTINUE; END IF;

    IF v_repair.outcome = 'restored'
      AND v_repair.applied_to_date IS NOT NULL
      AND v_repair.applied_to_date IS DISTINCT FROM v_restore_date
    THEN
      -- Remove only the protection row created by the repair migration. Rows
      -- from a real relic use predate the repair audit and are left untouched.
      DELETE FROM public.streak_freezers freezer
      WHERE freezer.ctid IN (
        SELECT candidate.ctid
        FROM public.streak_freezers candidate
        WHERE candidate.user_id = v_repair.user_id
          AND candidate.source = 'relic'
          AND candidate.applied_to_date = v_repair.applied_to_date
          AND candidate.used_at IS NULL
          AND candidate.purchased_at >= v_repair.repair_created_at - interval '1 minute'
        ORDER BY candidate.purchased_at
        LIMIT 1
      );
    END IF;

    INSERT INTO public.streak_freezers (user_id, freezer_type, source, applied_to_date)
    SELECT v_repair.user_id, 'weekly', 'relic', v_restore_date
    WHERE NOT EXISTS (
      SELECT 1 FROM public.streak_freezers existing
      WHERE existing.user_id = v_repair.user_id
        AND existing.used_at IS NULL
        AND existing.applied_to_date = v_restore_date
    );

    UPDATE public.relic_recovery_repairs
    SET applied_to_date = v_restore_date,
        outcome = 'restored'
    WHERE usage_ledger_id = v_repair.usage_ledger_id;
  END LOOP;
END;
$$;
