/*
  Complete Thief's Request recovery.

  Earlier versions sometimes awarded the denarii/talent part of the relic while
  failing to find the streak-breaking day. This migration makes the gap finder
  use the final preserved streak rules, routes future Thief's Request uses
  through that finder, and re-audits every historical Thief's Request use.
  Repairs are bounded by the original use time, so a relic used last week never
  repairs a streak loss that happened later.
*/
...
BEGIN
  SELECT LEAST(
    COALESCE((profile.created_at AT TIME ZONE 'Africa/Douala')::date, p_cutoff_date),
    COALESCE((
  ...
  IF v_start IS NULL OR p_cutoff_date < v_start THEN
    RETURN NULL;
  END IF;
...
REVOKE ALL ON FUNCTION public.find_latest_recoverable_streak_gap(uuid, date)
...
BEGIN
  ...
  IF v_relic.effect_type IN ('revive_lost_streak', 'resurrect_lost_streak') THEN
    ...
    SELECT p_user_id, 'weekly', 'relic', v_lost_streak_date
    WHERE NOT EXISTS (
      SELECT 1
      FROM public.streak_freezers existing
      WHERE existing.user_id = p_user_id
        AND existing.used_at IS NULL
        AND existing.applied_to_date = v_lost_streak_date
    );
...
BEGIN
  ...
  LOOP
    ...
    v_restore_date := public.find_latest_recoverable_streak_gap(
      v_usage.user_id,
      v_cutoff_date
    );

    SELECT repair.applied_to_date
    ...
    IF v_restore_date IS NULL THEN
      ...
      SET outcome = 'no_recoverable_gap',
          applied_to_date = NULL;
      CONTINUE;
    END IF;

    IF v_existing_date IS NOT NULL AND v_existing_date IS DISTINCT FROM v_restore_date THEN
      DELETE FROM public.streak_freezers freezer
      WHERE freezer.ctid IN (
        SELECT candidate.ctid
        FROM public.streak_freezers candidate
        WHERE candidate.user_id = v_usage.user_id
          AND candidate.source = 'relic'
          AND candidate.used_at IS NULL
          AND candidate.applied_to_date = v_existing_date
    ...
    INSERT INTO public.streak_freezers (user_id, freezer_type, source, applied_to_date)
    SELECT v_usage.user_id, 'weekly', 'relic', v_restore_date
    WHERE NOT EXISTS (
      SELECT 1
      FROM public.streak_freezers existing
      WHERE existing.user_id = v_usage.user_id
        AND existing.used_at IS NULL
