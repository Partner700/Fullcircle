/*
# Attendance reward and two-part weekday streak

- Sentries mark morning call attendance through one RPC.
- Present attendance grants the cadet 200 denarii once per date.
- Weekday streak validity requires both present attendance and daily devotion.
*/
...
BEGIN
  ...
  LOOP
    ...
    SELECT dr.meditation_submitted, dr.attendance_status
    INTO v_med, v_attendance_status
    ...
    v_day_complete := COALESCE(v_med, false) = true
...
SET streak_valid = CASE
  WHEN dr.day_type = 'sunday' THEN NULL
  WHEN dr.day_type = 'weekday' THEN
    COALESCE(dr.attendance_status, 'unmarked') = 'present'
    AND COALESCE(dr.meditation_submitted, false) = true
  ELSE COALESCE(dr.quiz_attempt_id IS NOT NULL, false)
END;

INSERT INTO public.denarii_ledger_entries (user_id, amount, source_type, source_reference, description)
SELECT dr.user_id, 200, 'attendance', dr.record_date::text, 'Attendance reward'
FROM public.daily_records dr
WHERE dr.attendance_status = 'present'
  AND NOT EXISTS (
    SELECT 1
    FROM public.denarii_ledger_entries dle
    WHERE dle.user_id = dr.user_id
      AND dle.source_type = 'attendance'
      AND dle.source_reference = dr.record_date::text
      AND dle.amount = 200
  );
